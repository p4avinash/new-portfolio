...
const ExperienceCard = ({ role, company, location, period, highlights, tech, isFirst }) => {
  return (
    <div className='relative pl-8 sm:pl-10 pb-12 last:pb-0 group'>
      ...
      <div className='absolute left-3 top-3 -bottom-3 w-0.5 bg-slate-800 group-last:hidden'></div>

      {/* Timeline indicator node */}
      <div className={`absolute left-0 top-1.5 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${
        isFirst 
          ? 'bg-emerald-500/20 border-emerald-400 text-emerald-400 shadow-md shadow-emerald-500/20 scale-110' 
      ...
      <article className='glass-card p-6 sm:p-8 rounded-2xl space-y-4'>
        {/* Header: Role & Period */}
        <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/80 pb-4'>
          ...
          <span className='inline-flex items-center gap-1.5 text-xs font-mono text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-full border border-emerald-500/20 self-start sm:self-center'>
        ...
        {tech && tech.length > 0 && (
          <div className='pt-4 border-t border-slate-800/80 flex flex-wrap gap-2'>
            {tech.map((item, idx) => (
              <span
                key={idx}
                className='text-xs font-mono px-2.5 py-1 rounded-md bg-slate-900 text-slate-300 border border-slate-800 hover:border-emerald-500/40 transition-colors'
              >
                {item}
              </span>
            ))}
